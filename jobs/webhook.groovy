                ...
                script {
                    ...
                    container(name: 'ansible') {
                        withVault([vaultSecrets: [[path: '/secret/automation/ssh-key', secretValues: [[envVar: 'ANSIBLE_SSH_KEY', vaultKey: 'ssh-key']]]]]) {
                            ...
                            sh "ansible-playbook -i ansible/kubevirt.yaml ansible/generic-webhook.yaml --extra-vars \"ansible_ssh_private_key_file=id_rsa\" -l namespace_${namespace} -l label_cluster_${cluster}"
                            sh 'rm id_rsa'
                        }
                    }
                }
